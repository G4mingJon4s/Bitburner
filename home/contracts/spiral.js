 ...
 */
 export async function main(ns) {
    ...
    ns.print(await spiral(ns, input));
}

/**
 * @param {import('../.vscode/NetscriptDefinitions').NS} ns 
 */
async function spiral(ns, input) {
    ...
    let result = [];
    let count = 0;
    while (result.length < (row + 1) * (col + 1) && count < 1000) {
...
function isNotPossible(pos, dir, prev, col, row) {
    return (pos[0] + dir[0] > col || pos[0] + dir[0] < 0 || pos[1] + dir[1] > row || pos[1] + dir[1] < 0 || prev.includes([pos[0] + dir[0], pos[1] + dir[1]]));
}
