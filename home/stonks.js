...
export async function main(ns) {
	...
	ns.print(
		`${
			COLOURS.green
		}Starting. Allow shorting: ${canShort}. Money percentage: ${percentage}%. Money on start: ${nFormatter(
			money
		)}$.${COLOURS.defaultG}`
	)
	await ns.sleep(1000)
	...
	while (true) {
		...
		if (entry === undefined) {
			const tail = ns
				...
				.reverse()
				.map((s) =>
					s.replace(/^\[(\d{2}:){2}\d{2} (\d{2}-){2}\d{4}\]/, "").trim()
				)
			ns.clearLog()

			if (/^ERROR: Could not find a viable stock! \([0-9]+\)$/.test(tail[0])) {
				tail[0] = `ERROR: Could not find a viable stock! (${
					parseInt(tail[0].match(/(\d+)/)[0]) + 1
				})`
			} else tail.unshift("ERROR: Could not find a viable stock! (0)")
...
export function findBestStock(
	...
	const data = symbols.map((x, i) => {
		...
		const score = price * Math.pow(Math.abs(0.5 - foreCast), 1.45)
		return {
			symbol,
			foreCast,
			shares,
...
export class Stock {
	...
	constructor(
		...
		this.short = short
		this.profit =
			this.shareCount * (this.sellPrice - this.buyPrice) * (this.short ? -1 : 1)
		this.visualInd = true
